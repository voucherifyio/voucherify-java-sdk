...
public class DistributionsExample extends AbsExample {
  ...
  public void example() {
    ...
    Customer custToPublish = Customer.builder().id(customerResponse.getId()).build();

    PublishVoucher publishVoucher =
        PublishVoucher.builder().voucher(result.getCode()).customer(custToPublish).build();

    System.out.println(client.distributions().publish(publishVoucher));

    CreateExport createExport =
        CreateExport.builder()
            .exportedObject("voucher")
            .parameters(ExportParams.builder().field("code").field("voucher_type").build())
            .build();
